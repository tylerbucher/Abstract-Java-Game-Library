...
public abstract class OctaTree<O> {
    
    private OctaTreeNode<O> rootNode;
    
    public void insert(O object) {
        insert(object, rootNode);
    }
    
    private void insert(O object, OctaTreeNode<O> node) {
        if(node.parent) {
            for(int i=0;i<8;i++)
        ...
        if(node.objectList.size() > OctaTreeNode.maxObjects && node.currentLevel < OctaTreeNode.maxLevels) {
            node.split();
    ...
    private void remove(O object, OctaTreeNode<O> node) {
        if(node.parent) {
            for(int i=0;i<8;i++)
                if(intersects(object, node.children[i]))
    ...
    private boolean isObjectColliding(O object, OctaTreeNode<O> node) {
        if(node.parent) {
            for(int i=0;i<8;i++)
                if(intersects(object, node.children[i]))
    ...
    private void getPossibleObjectCollisions(O object, OctaTreeNode<O> node, List<O> collisionList) {
        if(node.parent) {
            for(int i=0;i<8;i++)
                if(intersects(object, node.children[i]))
    ...
    private void getCollisions(O object, OctaTreeNode<O> node, List<O> collisionList) {
        if(node.parent) {
            for(int i=0;i<8;i++)
                if(intersects(object, node.children[i]))
    ...
    public abstract boolean intersects(O object, OctaTreeNode<O> node);
}
